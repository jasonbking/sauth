 ...
 */
	...
	server.put({
		name: 'addUser',
		path: '/users'
	}, perm('create', 'sauth:user'), addUser);

	server.del({
		name: 'deleteUser',
		path: '/users/:id'
	}, perm('delete', 'sauth:user'), deleteUser);
	...
	}, perm('group', 'sauth:user'), addUserToGroup);

	server.del({
		name: 'removeUserFromGroup',
		path: '/users/:id/groups/:gid'
	}, perm('group', 'sauth:user'), removeUserFromGroup);
	...
	server.put({
		name: 'addUserRule',
		path: '/users/:id/rules'
	}, perm('rule', 'sauth:user'), addUserRule);

	server.del({
		name: 'removeUserRule',
		path: '/users/:id/rules/:rid'
	}, perm('rule', 'sauth:user'), removeUserRule);
    ...
    server.put({
		name: 'addGroup',
		path: '/groups'
	}, perm('create', 'sauth:group'), addGroup);

	server.put({
		name: 'addGroupMember',
		path: '/groups/:id/users/:uid'
	}, perm('group', 'sauth:user'), addGroupMember);

	server.del({
		name: 'removeGroupMember',
		path: '/groups/:id/users/:uid'
	}, perm('group', 'sauth:user'), removeGroupMember);
...
function addAccount(req, res, next) {
	if (!req.params.name) {
		next(new restify.BadRequestError('No account name provided'));
		return;
	}

	req.log.debug('create account \'' + req.params.name + '\'');
	req.sauth.db.accounts.add(req.params.name)
		.then(function (data) {
			req.log.info('created account ' + data.name + ' '
				+ data.accountUuid);
			return data;
		})
		.then(success(req, res, next))
...
function deleteAccount(req, res, next) {
	req.sauth.db.accounts.remove(req.params.id)
		.then(function (count) {
			if (count > 0) {
				req.log.info('deleted account ' + req.params.id);
			}
			return {};
		})
		.then(success(req, res, next))
...
function addGroup(req, res, next) {
	if (!req.params.name) {
		next(new restify.BadRequestError('no group name specified'));
		return;
	}

	req.log.debug('Creating group ' + req.params.name);
	req.sauth.db.groups.add(req.header('accountuuid'), req.params.name)
		.then(function (data) {
			req.log.debug('Created group ' + data.groupUuid + ' ('
				+ req.name + ')');
			return data;
		})
		.then(success(req, res, next))
...
// used as a handler function to require the given
// permissions from a user
function perm(action, target) {
	return function (req, res, next) {
		check(action, target, req)
			.then(function (allow) {
				if (!allow) {
